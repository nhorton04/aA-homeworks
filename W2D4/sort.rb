...
class SortingDemo
  ...
  def self.merge(left, right, &prc)
    ...
    p merged_array + left + right
    merged_array + left + right
  end

  def self.performance_test(size, count)
    p arrays_to_test = Array.new(count) { random_arr(size) }
  ...
  def self.run_performance_tests(multiplier = 5941, rounds = 3)
